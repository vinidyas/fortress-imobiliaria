...
const peopleList = ref<PersonOption[]>(props.people?.map(normalizePersonOption) ?? []);

if (import.meta.env.DEV) {
  console.debug('[TransactionForm] initial people', peopleList.value.length);
}
...
watch(
  () => props.people,
  (newValue) => {
    mergePeople((newValue ?? []).map(normalizePersonOption));
  },
  { deep: true }
);
