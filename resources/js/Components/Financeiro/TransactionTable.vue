...
const handleClone = async (item: TransactionRow) => {
  if (!props.can.create) {
    return;
  }
  ...
  try {
    await axios.post(`/api/financeiro/journal-entries/${item.id}/clone`, {
      movement_date: new Date().toISOString().slice(0, 10),
    });
    toast.success('Lançamento clonado com sucesso.');
    reloadPage();
  } catch (error: any) {
    const message = error?.response?.data?.message ?? 'Não foi possível clonar o lançamento.';
    toast.error(message);
  } finally {
